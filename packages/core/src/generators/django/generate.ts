...
const blockToDjango = (
  ...
  if (checkIsForNode(json)) {
    ...
    const expression = `${processBinding(json.bindings.each?.code as string)}?.map((${forArgs}) => (
      ...
      .map((item) => blockToDjango(item, options, wrap))
      .join('\n')}${wrap ? '</>' : ''}
    ))`;
    if (insideJsx) {
      return `{${expression}}`;
    ...
    const expression = `${processBinding(json.bindings.when?.code as string)} ? (
      ...
      .map((item) => blockToDjango(item, options, wrap))
      .join('\n')}${wrap ? '</>' : ''}
    ) : ${!json.meta.else ? 'null' : blockToDjango(json.meta.else as any, options, false)}`;
  ...
  str += '>';
  if (json.children) {
    str += json.children.map((item) => blockToDjango(item, options, true)).join('\n');
  }
...
export const componentToDjango: TranspilerGenerator<ToDjangoOptions> =
  (_options = {}) =>
  ({ component }) => {
    const options = initializeOptions({ target: 'django', component, defaults: _options });
    ...
    export default class ${json.name} {
      ...
      render() {
        return (${wrap ? '<>' : ''}
          ...
          ${json.children.map((item) => blockToDjango(item, options, true)).join('\n')}
